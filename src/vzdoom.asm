; ============================================================================
; vzdoom.asm — VZ-DOOM Milestone 4: doors, gun, sound
; ============================================================================
; M3 (verified) plus:
;   - Doors: map cell 2, rendered $DD (red/yellow panels) on both faces.
;     E opens the door ~0.5 cell ahead; it auto-closes after ~5s unless the
;     player is standing in it. Rays and movement treat closed doors as walls.
;   - Weapon: 16x16 masked pistol sprite at bottom centre, muzzle flash for
;     4 frames when firing (SPACE), 10-frame cooldown.
;   - Sound: 1-bit speaker via VDC latch bits 0/5 (push-pull), preserving
;     bit3 (mode) and bit4 (CSS). Footstep click every 8th moving frame,
;     door whoosh, fire blip. Every write is also visible to the Pico.
;
; Controls: W/S move · A/D turn · ,/. strafe · E open door · SPACE fire
;
; Keyboard rows (bit order hardware-verified):
;   $68FE: b1=W b3=E     $68FD: b1=S b3=D b4=A     $68EF: b1=. b3=, b4=SPACE
;
; Build:  ..\build.ps1 src\vzdoom.asm VZDOOM
; ============================================================================
...
START:
        ...
        LD      A, 100
        LD      (HLTH), A
        LD      A, 255          ; force the first HUD draw
        LD      (LHLTH), A
...
.noai:
        ...
        OR      A
        JR      NZ, .living
.dead:  LD      A, (KROW0)      ; dead: freeze until Q
        AND     $10
        JR      NZ, .dead
        XOR     A
        LD      (LATCH), A
...
BANKEND:
        ...
        SAVEBIN "build/vzdoom.bin", START, BANKEND - START
